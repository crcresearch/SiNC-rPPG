...
def main(args):
    ...
    for session in sessions:
        ...
        gt_path = join(input_root, session, "ground_truth.txt")
        wave, HR, wave_t = np.loadtxt(gt_path)
        lmrks = get_face_detector(args.detector).landmark_video(video_path)
        ...
        if successful:
            ...
            print(output_path)
            np.savez_compressed(
                output_path, video=output_video, wave=wave, video_path=video_path, fps=30
            )


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("input", help="Path to the original UBFC-rPPG dataset directory.")
    parser.add_argument(
