...
def IPR_SSL(freqs, psd, speed=None, low_hz=BP_LOW, high_hz=BP_HIGH, device=None):
    if speed is None:
        ...
        for b in range(batch_size):
            ...
            ipr_losses[b] = _IPR_SSL(
                freqs, psd_b, low_hz=low_hz_b, high_hz=high_hz_b, device=device
            )
...
def _EMD_SSL(
    freqs, psd, low_hz=BP_LOW, high_hz=BP_HIGH, normalized=False, bandpassed=False, device=None
):
...
def EMD_SSL(
    ...
    if speed is None:
        ...
        emd_loss = torch.mean(
            torch.square(torch.cumsum(psd, dim=0) - torch.cumsum(expected, dim=0))
        )
    return emd_loss
...
def SNR_harmonic_SSL(
    ...
    if speed is None:
        ...
        for b in range(batch_size):
            low_hz_b = low_hz * speed[b]
            high_hz_b = high_hz * speed[b]
            snr_losses[b] = _SNR_harmonic_SSL(
                freqs,
                psd[b].view(1, -1),
                low_hz=low_hz_b,
                high_hz=high_hz_b,
...
def torch_power_spectral_density(
    x,
    nfft=5400,
    fps=90,
    low_hz=BP_LOW,
