...
from utils.losses import normalize_psd, torch_power_spectral_density
...
def unsupervised_train_step(model, data, criterions, device, arg_obj):
    ...
    freqs, psd = torch_power_spectral_density(
        predictions, fps=fps, low_hz=low_hz, high_hz=high_hz, normalize=False, bandpass=False
    )
...
def accumulate_validation_losses(freqs, psd, criterions, device, arg_obj):
    ...
    if "b" in criterions_str:
        bandwidth_loss = criterions["bandwidth"](
            freqs, psd, low_hz=low_hz, high_hz=high_hz, device=device
        )
